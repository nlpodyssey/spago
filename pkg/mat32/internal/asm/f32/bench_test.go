...
func benchaxpyinc(t *testing.B, ln, tInc int, f func(alpha float32, x, y []float32, n, incX, incY, ix, iy uintptr)) {
	n, inc := uintptr(ln), uintptr(tInc)
	var idx int
	if tInc < 0 {
		idx = (-ln + 1) * tInc
	}
...
func benchaxpyincto(t *testing.B, ln, tInc int, f func(dst []float32, incDst, idst uintptr, alpha float32, x, y []float32, n, incX, incY, ix, iy uintptr)) {
	n, inc := uintptr(ln), uintptr(tInc)
	var idx int
	if tInc < 0 {
		idx = (-ln + 1) * tInc
...
func BenchmarkLF32AxpyIncToN100000IncM10(b *testing.B) {
