...
func Dgemm(aTrans, bTrans bool, m, n, k int, alpha float64, a []float64, lda int, b []float64, ldb int, beta float64, c []float64, ldc int) {
	...
	if n < 0 {
		panic("dgemm: invalid n < 0")
	}
	if k < 0 {
		panic("dgemm: invalid k < 0")
	}

	if aTrans {
		if lda < max(1, m) {
			panic("dgemm: invalid lda")
		}
	} else {
		if lda < max(1, k) {
			panic("dgemm: invalid lda")
	...
	if bTrans {
		...
		if len(b) < (k-1)*ldb+n {
			panic("dgemm: wrong b size")
		}
	}
	if len(c) < (m-1)*ldc+n {
		panic("dgemm: wrong c size")
	}
