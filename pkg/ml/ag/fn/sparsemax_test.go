...
func TestSparseMax_Forward(t *testing.T) {
	...
	assert.InDeltaSlice(t, []float64{0.3597, 0.0138, 0.0000, 0.0000, 0.6265}, y.Data(), 1.0e-3)

	f.Backward(mat.NewVecDense([]float64{1.0, 0.5, 0.5, 0.5, 1.0, 1.0}))

	assert.InDeltaSlice(t, []float64{0.16, -0.33, 0, 0, 0.16}, x.grad.Data(), 1.0e-2)
}
...
func TestSparseMaxLoss_Forward(t *testing.T) {
	...
	y := f.Forward()
	assert.InDeltaSlice(t, []float64{-1.3009, -0.2396, -1.2110, -0.7479, -0.2606}, y.Data(), 1.0e-2)

	f.Backward(mat.NewVecDense([]float64{0, 0, -1, 0, 0}))

	assert.InDeltaSlice(t, []float64{0.0000, 0.5098, -1.0000, 0.0015, 0.4888}, x.grad.Data(), 1.0e-2)
}
