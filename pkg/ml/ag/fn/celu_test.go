...
func TestCELUForward(t *testing.T) {
	...
	f := NewCELU[mat.Float](x, alpha)
	y := f.Forward()

	assert.InDeltaSlice(t, []mat.Float{0.1, -0.19032516, 0.3, 0}, y.Data(), 1.0e-6)

	f.Backward(mat.NewVecDense([]mat.Float{-1.0, 0.5, 0.8, 0.0}))

	assert.InDeltaSlice(t, []mat.Float{-1.0, 0.45241870, 0.8, 0.0}, x.grad.Data(), 1.0e-6)
}
