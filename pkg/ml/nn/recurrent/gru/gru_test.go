...
func TestModel_Forward(t *testing.T) {
	...
	assert.InDeltaSlice(t, []float64{0.74, -0.23, 0.11, 0.49, -0.05}, y.Value().Data(), 0.005)
	...
	assert.InDeltaSlice(t, []float64{0.53, -0.49, 0.18, 0.20}, x.Grad().Data(), 0.005)
	...
	assert.InDeltaSlice(t, []float64{0.02, 0.13, 0.03, -0.27, 0.02}, model.BPart.Grad().Data(), 0.005)
	...
	assert.InDeltaSlice(t, []float64{0.04, -0.3, 0.0, -0.07, -0.12}, model.BCand.Grad().Data(), 0.005)
...
func TestModel_ForwardWithPrev(t *testing.T) {
	...
	assert.InDeltaSlice(t, []float64{0.86, 0.18, -0.24, 0.36, -0.36}, y.Value().Data(), 0.005)
	...
	assert.InDeltaSlice(t, []float64{0.56, -0.83, 0.5, 0.55}, x.Grad().Data(), 0.005)
	...
	assert.InDeltaSlice(t, []float64{0.03, 0.01, -0.01, -0.52, -0.22}, model.BPart.Grad().Data(), 0.005)
	...
	assert.InDeltaSlice(t, []float64{0.02, -0.1, 0.0, -0.06, -0.28}, model.BCand.Grad().Data(), 0.005)
	...
	assert.InDeltaSlice(t, []float64{-0.01, 0.0, -0.02, -0.03, -0.01}, model.BRes.Grad().Data(), 0.005)
...
func TestModel_ForwardSeq(t *testing.T) {
	...
	assert.InDeltaSlice(t, []float64{-0.634733134450701, 0.896135841414256}, s.Y.Value().Data(), 1.0e-05)

	x2 := g.NewVariable(mat.NewVecDense([]float64{3.3, -2.0, 0.1}), true)
	_ = proc.Forward(x2)
	s2 := proc.LastState()

	assert.InDeltaSlice(t, []float64{0.646126994447876, 0.537141024639326}, s2.Y.Value().Data(), 1.0e-05)
	...
	assert.InDeltaSlice(t, []float64{0.022626682234541, 0.019282896989004, -0.05477940973827}, x.Grad().Data(), 1.0e-05)
	assert.InDeltaSlice(t, []float64{0.047347465801696, 0.102160284950441, -0.023609485283631}, x2.Grad().Data(), 1.0e-05)
