...
func TestModel_Forward(t *testing.T) {
	...
	assert.InDeltaSlice(t, []float64{0.287518, 0.06939, -0.259175, 0.20769, -0.263768}, y.Value().Data(), 1.0e-05)
	...
	assert.InDeltaSlice(t, []float64{0.023319, 0.253729, -0.122248, 0.190719}, x.Grad().Data(), 1.0e-06)
	...
	assert.InDeltaSlice(t, []float64{-0.091124, -0.095413, -0.057328, -0.258489, -0.318553}, model.B.Grad().Data(), 1.0e-06)
	assert.InDeltaSlice(t, []float64{-0.0368, -0.039102, 0.008963, -0.194915, 0.071569}, model.BPart.Grad().Data(), 1.0e-06)
	assert.InDeltaSlice(t, []float64{0.074722, 0.104, -0.017198, -0.018094, -0.066896}, model.Beta1.Grad().Data(), 1.0e-06)
...
func TestModel_ForwardWithPrev(t *testing.T) {
	...
	assert.InDeltaSlice(t, []float64{0.202158, 0.228591, -0.240679, -0.350224, -0.476828}, y.Value().Data(), 1.0e-05)
	...
	assert.InDeltaSlice(t, []float64{-0.177606, 0.379355, -0.085751, 0.080693}, x.Grad().Data(), 1.0e-06)
	...
	assert.InDeltaSlice(t, []float64{-0.122505, -0.06991, -0.054249, -0.493489, -0.353592}, model.B.Grad().Data(), 1.0e-06)
	assert.InDeltaSlice(t, []float64{-0.06814, -0.0145, -0.001299, -0.413104, -0.041468}, model.BPart.Grad().Data(), 1.0e-06)
	assert.InDeltaSlice(t, []float64{0.100454, 0.076202, -0.016275, -0.034544, -0.074254}, model.Beta1.Grad().Data(), 1.0e-06)
	assert.InDeltaSlice(t, []float64{-0.135487, 0.002895, -0.009628, -0.251233, -0.097111}, model.Beta2.Grad().Data(), 1.0e-06)
	assert.InDeltaSlice(t, []float64{0.1111, -0.003156, -0.002889, -0.017586, -0.020393}, model.Alpha.Grad().Data(), 1.0e-06)
}
...
func TestModel_ForwardSeq(t *testing.T) {
	...
	assert.InDeltaSlice(t, []float64{0.176979535, 0.7339353781}, s.Y.Value().Data(), 1.0e-05)

	x2 := g.NewVariable(mat.NewVecDense([]float64{3.3, -2.0, 0.1}), true)
	_ = proc.Forward(x2)
	s2 := proc.LastState()

	assert.InDeltaSlice(t, []float64{0.0060780253, 0.6727636037}, s2.Y.Value().Data(), 1.0e-05)
	...
	assert.InDeltaSlice(t, []float64{0.0002377894, 0.000303171, -0.0004869122}, x.Grad().Data(), 1.0e-05)

	assert.InDeltaSlice(t, []float64{-1.86775074391114e-005, -0.0004428228, 0.000880455}, x2.Grad().Data(), 1.0e-05)
