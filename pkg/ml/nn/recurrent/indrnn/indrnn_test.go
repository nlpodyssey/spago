...
func TestModel_Forward(t *testing.T) {
	...
	assert.InDeltaSlice(t, []float64{-0.39693, -0.796878, 0.0, 0.701374, -0.187746}, y.Value().Data(), 1.0e-05)

	// == Backward

	g.Backward(y, ag.OutputGrad(mat.NewVecDense([]float64{0.57, 0.75, -0.15, 1.64, 0.45})))

	assert.InDeltaSlice(t, []float64{1.166963, -0.032159, -0.705678, -0.318121}, x.Grad().Data(), 1.0e-05)
...
func TestModel_ForwardWithPrev(t *testing.T) {
	...
	assert.InDeltaSlice(t, []float64{-0.39693, -0.842046, 0.256335, 0.701374, 0.205456}, y.Value().Data(), 1.0e-05)

	// == Backward

	g.Backward(y, ag.OutputGrad(mat.NewVecDense([]float64{0.57, 0.75, -0.15, 1.64, 0.45})))

	assert.InDeltaSlice(t, []float64{1.133745, -0.019984, -0.706080, -0.271285}, x.Grad().Data(), 0.005)
...
func TestModel_ForwardSeq(t *testing.T) {
	...
	assert.InDeltaSlice(t, []float64{-0.602213565, 0.9898794918}, s2.Y.Value().Data(), 1.0e-05)
	...
	assert.InDeltaSlice(t, []float64{8.79795806788067e-005, 0.0001270755, -0.0002101123}, x.Grad().Data(), 1.0e-05)

	assert.InDeltaSlice(t, []float64{0.0004629577, 0.0005937435, -0.0009550013}, x2.Grad().Data(), 1.0e-05)
