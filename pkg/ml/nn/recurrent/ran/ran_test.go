...
func TestModel_Forward(t *testing.T) {
	...
	assert.InDeltaSlice(t, []float64{0.39652, 0.25162, 0.5, 0.70475, 0.45264}, s.InG.Value().Data(), 1.0e-05)
	assert.InDeltaSlice(t, []float64{0.85321, 0.43291, 0.11609, 0.51999, 0.24232}, s.ForG.Value().Data(), 1.0e-05)
	assert.InDeltaSlice(t, []float64{1.02, -0.1, 0.1, 2.03, -1.41}, s.Cand.Value().Data(), 1.0e-05)
	assert.InDeltaSlice(t, []float64{0.38375, -0.02516, 0.04996, 0.8918, -0.56369}, s.Y.Value().Data(), 1.0e-05)
	...
	assert.InDeltaSlice(t, []float64{0.21996, -0.12731, 0.10792, 0.49361}, x.Grad().Data(), 1.0e-05)
	...
	assert.InDeltaSlice(t, []float64{-0.03877, 0.01459, 0.00499, -0.06469, 0.2416}, model.BIn.Grad().Data(), 1.0e-05)
	...
	assert.InDeltaSlice(t, []float64{-0.06298, -0.19492, 0.09973, -0.10794, -0.31304}, model.BCand.Grad().Data(), 1.0e-05)
...
func TestModel_ForwardWithPrev(t *testing.T) {
	...
	assert.InDeltaSlice(t, []float64{0.72312, 0.24974, 0.54983, 0.82054, 0.53494}, s.InG.Value().Data(), 1.0e-05)
	assert.InDeltaSlice(t, []float64{0.91133, 0.18094, 0.04834, 0.67481, 0.38936}, s.ForG.Value().Data(), 1.0e-05)
	assert.InDeltaSlice(t, []float64{1.02, -0.1, 0.1, 2.03, -1.41}, s.Cand.Value().Data(), 1.0e-05)
	assert.InDeltaSlice(t, []float64{0.5045, 0.01121, 0.04046, 0.78504, -0.78786}, s.Y.Value().Data(), 1.0e-05)
	...
	assert.InDeltaSlice(t, []float64{0.19694, 0.11428, -0.14008, 0.15609}, x.Grad().Data(), 1.0e-05)
	...
	assert.InDeltaSlice(t, []float64{-0.00997, 0.01384, 0.00471, -0.09807, 0.16469}, model.BIn.Grad().Data(), 1.0e-05)
	...
	assert.InDeltaSlice(t, []float64{-0.03531, -0.18448, 0.10455, -0.26919, -0.25115}, model.BCand.Grad().Data(), 1.0e-05)
...
func TestModel_ForwardSeq(t *testing.T) {
	...
	assert.InDeltaSlice(t, []float64{-0.109472457893732, 0.998622353363888}, s.Y.Value().Data(), 1.0e-05)

	x2 := g.NewVariable(mat.NewVecDense([]float64{3.3, -2.0, 0.1}), true)
	_ = proc.Forward(x2)
	s2 := proc.LastState()

	assert.InDeltaSlice(t, []float64{0.331268038964841, 0.573215090496922}, s2.Y.Value().Data(), 1.0e-05)
	...
	assert.InDeltaSlice(t, []float64{-0.00020471646083, 0.000980786890846, -0.000569825073637}, x.Grad().Data(), 1.0e-05)
	assert.InDeltaSlice(t, []float64{0.106092179481869, 0.139102385565363, -0.107968562632196}, x2.Grad().Data(), 1.0e-05)
