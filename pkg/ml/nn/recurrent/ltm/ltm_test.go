...
func TestModel_Forward(t *testing.T) {
	...
	assert.InDeltaSlice(t, []float64{0.531299, 0.439948, 0.484336, 0.443710}, s.Cell.Value().Data(), 1.0e-06)

	assert.InDeltaSlice(t, []float64{0.368847, 0.208984, 0.354078, 0.350904}, s.Y.Value().Data(), 1.0e-06)
	...
	assert.InDeltaSlice(t, []float64{0.22696676, 0.009912126, -0.105133662, -0.040795301}, x.Grad().Data(), 1.0e-06)
...
func TestModel_ForwardWithPrev(t *testing.T) {
	...
	assert.InDeltaSlice(t, []float64{0.633246, 0.276811, 0.474442, 0.256067}, s.Cell.Value().Data(), 1.0e-06)

	assert.InDeltaSlice(t, []float64{0.379117, 0.106466, 0.381340, 0.191636}, s.Y.Value().Data(), 1.0e-06)
	...
	assert.InDeltaSlice(t, []float64{0.199768, 0.0135238, -0.0872397, -0.034826}, x.Grad().Data(), 1.0e-06)
	assert.InDeltaSlice(t, []float64{0.199768, 0.0135238, -0.0872397, -0.034826}, yPrev.Grad().Data(), 1.0e-06)
	assert.InDeltaSlice(t, []float64{0.144275, 0.060885, 0.226774, -0.08242}, cellPrev.Grad().Data(), 1.0e-06)
...
func TestModel_ForwardSeq(t *testing.T) { //TODO FIX TEST
	...
	assert.InDeltaSlice(t, []float64{0.6585208524, 0.5193369948, 0.3051361057}, s.Cell.Value().Data(), 1.0e-05)

	assert.InDeltaSlice(t, []float64{0.1052066064, 0.445668489, 0.0920091497}, s.Y.Value().Data(), 1.0e-05)

	x2 := g.NewVariable(mat.NewVecDense([]float64{3.3, -2.0, 0.1}), true)
	_ = proc.Forward(x2)
	s2 := proc.LastState()

	assert.InDeltaSlice(t, []float64{0.7639620348, 0.5509132249, 0.1590346479}, s2.Cell.Value().Data(), 1.0e-05)

	assert.InDeltaSlice(t, []float64{0.0492343522, 0.4588769062, 0.0947403852}, s2.Y.Value().Data(), 1.0e-05)
	...
	assert.InDeltaSlice(t, []float64{0.0058833, 0.0008477, -0.0048020}, x.Grad().Data(), 1.0e-05)
	assert.InDeltaSlice(t, []float64{-0.02891086, 0.00700993, 0.0099656}, x2.Grad().Data(), 1.0e-05)
