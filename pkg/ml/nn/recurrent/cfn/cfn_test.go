...
func TestModel_Forward(t *testing.T) {
	...
	assert.InDeltaSlice(t, []float64{0.268, -0.025, 0.381, 0.613, -0.364}, y.Value().Data(), 0.0005)
	...
	assert.InDeltaSlice(t, []float64{0.318, 0.01, -0.027, 0.302}, x.Grad().Data(), 0.005)
	...
	assert.InDeltaSlice(t, []float64{-0.049, 0.015, 0.101, -0.186, 0.16}, model.BIn.Grad().Data(), 0.005)
...
func TestModel_ForwardWithPrev(t *testing.T) {
	...
	assert.InDeltaSlice(t, []float64{0.308, 0.011, 0.405, 0.230, -0.689}, y.Value().Data(), 0.0005)
	...
	assert.InDeltaSlice(t, []float64{0.111, 0.37, -0.281, 0.126}, x.Grad().Data(), 0.005)
	...
	assert.InDeltaSlice(t, []float64{-0.035, 0.014, 0.105, -0.18, 0.228}, model.BIn.Grad().Data(), 0.005)
	...
	assert.InDeltaSlice(t, []float64{0.004, -0.022, -0.007, 0.222, 0.18}, model.BFor.Grad().Data(), 0.005)
...
func TestModel_ForwardSeq(t *testing.T) {
	...
	assert.InDeltaSlice(t, []float64{-0.0886045623, 0.9749300057}, s.Y.Value().Data(), 1.0e-05)

	x2 := g.NewVariable(mat.NewVecDense([]float64{3.3, -2.0, 0.1}), true)
	_ = proc.Forward(x2)
	s2 := proc.LastState()

	assert.InDeltaSlice(t, []float64{0.2205790544, 0.5834192006}, s2.Y.Value().Data(), 1.0e-05)
	...
	assert.InDeltaSlice(t, []float64{0.0087725508, 0.0021613524, 0.000922185}, x.Grad().Data(), 1.0e-05)

	assert.InDeltaSlice(t, []float64{0.1519336751, 0.2004414748, -0.1394754602}, x2.Grad().Data(), 1.0e-05)
