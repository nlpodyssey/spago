...
func TestModel_ForwardWithPrev(t *testing.T) {
	...
	assert.InDeltaSlice(t, []float64{0.88, -1.1, -0.45, 0.41}, s0.Y.Value().Data(), 1.0e-05)

	x1 := g.NewVariable(mat.NewVecDense([]float64{0.8, -0.3, 0.5, 0.3}), true)
	_ = proc.Forward(x1)
	s1 := proc.LastState()

	assert.InDeltaSlice(t, []float64{0.5996537, -0.545537, -0.63689751, 0.453609420}, s1.Y.Value().Data(), 1.0e-05)
}
