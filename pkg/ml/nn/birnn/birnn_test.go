...
func TestModelConcat_Forward(t *testing.T) {
	...
	assert.InDeltaSlice(t, []float64{1.031472, -0.627913}, x1.Grad().Data(), 0.006)
	assert.InDeltaSlice(t, []float64{-0.539497, -0.629167}, x2.Grad().Data(), 0.006)
	assert.InDeltaSlice(t, []float64{0.013097, -0.09932}, x3.Grad().Data(), 0.006)
...
func TestModelSum_Forward(t *testing.T) {
	...
	assert.InDeltaSlice(t, []float64{0.182469, -0.584826, -0.519207}, y[0].Value().Data(), 1.0e-06)
	assert.InDeltaSlice(t, []float64{-1.033731, -0.036692, -0.513732}, y[1].Value().Data(), 1.0e-06)
	assert.InDeltaSlice(t, []float64{0.385793, 0.008929, 0.517957}, y[2].Value().Data(), 1.0e-06)
}
...
func TestModelAvg_Forward(t *testing.T) {
	...
	assert.InDeltaSlice(t, []float64{0.0912345, -0.292413, -0.2596035}, y[0].Value().Data(), 1.0e-06)
	assert.InDeltaSlice(t, []float64{-0.5168655, -0.018346, -0.256866}, y[1].Value().Data(), 1.0e-06)
	assert.InDeltaSlice(t, []float64{0.1928965, 0.0044645, 0.2589785}, y[2].Value().Data(), 1.0e-06)
}
...
func TestModelProd_Forward(t *testing.T) {
	...
	assert.InDeltaSlice(t, []float64{-0.00099, 0.042197, -0.068886}, y[0].Value().Data(), 1.0e-06)
	assert.InDeltaSlice(t, []float64{0.231888, -0.047735, 0.028804}, y[1].Value().Data(), 1.0e-06)
	assert.InDeltaSlice(t, []float64{0.033161, -0.519478, -0.206044}, y[2].Value().Data(), 1.0e-06)
}
