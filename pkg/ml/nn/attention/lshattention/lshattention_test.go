...
func TestModel_LshAttention(t *testing.T) {
	...
	assert.InDeltaSlice(t, []float64{-0.0996905, 0.7098312, 0.49985933, -1.1750140}, output[0].Value().Data(), 1.0e-05)
	assert.InDeltaSlice(t, []float64{0.15378995, 0.37251139, 0.42739101, -0.46110926}, output[1].Value().Data(), 1.0e-05)
	assert.InDeltaSlice(t, []float64{-0.15323142, 0.10676857, 0.01016588, -0.59778175}, output[2].Value().Data(), 1.0e-05)
	assert.InDeltaSlice(t, []float64{0.1580695, 0.3598185, 0.41276363, -0.454396291}, output[3].Value().Data(), 1.0e-05)
	assert.InDeltaSlice(t, []float64{-0.14, 0.84, 0.48, -0.65}, output[4].Value().Data(), 1.0e-05)
	assert.InDeltaSlice(t, []float64{0.1583759, 0.35892780, 0.41173471, -0.4539261}, output[5].Value().Data(), 1.0e-05)
	assert.InDeltaSlice(t, []float64{-0.1002830, 0.7101543, 0.5001286, -1.1749871}, output[6].Value().Data(), 1.0e-05)
	assert.InDeltaSlice(t, []float64{0.1608603, 0.3508395, 0.4026476, -0.4492517}, output[7].Value().Data(), 1.0e-05)
	assert.InDeltaSlice(t, []float64{-0.1584371, 0.10156288, 0.02439478, -0.57487669}, output[8].Value().Data(), 1.0e-05)

	output[0].PropagateGrad(mat.NewVecDense([]float64{-0.04, 0.36, 0.32, 0.01}))

	g.Backward(output[0])

	assert.InDeltaSlice(t, []float64{0.07993073, -0.17005416, -0.1586937, -0.2616235, 0.1777840}, x1.Grad().Data(), 1.0e-05)
