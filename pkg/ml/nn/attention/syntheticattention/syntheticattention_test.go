...
func TestModel_SyntheticAttention(t *testing.T) {
	t.Run("float32", testModelSyntheticAttention[float32])
	t.Run("float64", testModelSyntheticAttention[float64])
}

func testModelSyntheticAttention[T mat.DType](t *testing.T) {
	...
	assert.InDeltaSlice(t, []T{0.778722, -0.5870107, -0.36687185}, output[0].Value().Data(), 1.0e-05)
	assert.InDeltaSlice(t, []T{0.8265906, -0.7254198, -0.3560310}, output[1].Value().Data(), 1.0e-05)
	assert.InDeltaSlice(t, []T{0.7651726, -0.5680948, -0.3797024}, output[2].Value().Data(), 1.0e-05)
	...
	assert.InDeltaSlice(t, []T{-0.0015012, 0.40111136, -0.26565675, -0.1677756}, x1.Grad().Data(), 1.0e-05)
	assert.InDeltaSlice(t, []T{0.0472872, 0.19752994, -0.14673837, -0.0747287}, x2.Grad().Data(), 1.0e-05)
	assert.InDeltaSlice(t, []T{0.02086887, 0.2511595, -0.1880584, -0.07230184}, x3.Grad().Data(), 1.0e-05)
	...
	}, model.FFN.Layers[0].(*linear.Model[T]).W.Grad().Data(), 1.0e-05)
	assert.InDeltaSlice(t, []T{
		-0.0779462, 0.0, -0.0093873, -0.04949895,
	}, model.FFN.Layers[0].(*linear.Model[T]).B.Grad().Data(), 1.0e-05)
	assert.InDeltaSlice(t, []T{
...
func newTestModel[T mat.DType]() *Model[T] {
	...
	model.FFN.Layers[0].(*linear.Model[T]).W.Value().SetData([]T{
		0.7, -0.2, -0.1, 0.2,
		-0.1, -0.1, 0.3, -0.2,
		0.6, 0.1, 0.9, 0.3,
		-0.3, 0.3, 0.4, -0.8,
	})
	model.FFN.Layers[0].(*linear.Model[T]).B.Value().SetData([]T{0.8, -0.2, -0.5, 0.2})
