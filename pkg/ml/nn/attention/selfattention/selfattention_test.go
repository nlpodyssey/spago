...
func TestModel_SelfAttention(t *testing.T) {
	...
	assert.InDeltaSlice(t, []float64{0.789110, -0.755551, -0.431247}, output[0].Value().Data(), 1.0e-05)
	assert.InDeltaSlice(t, []float64{0.780654, -0.6212001, -0.380214}, output[1].Value().Data(), 1.0e-05)
	assert.InDeltaSlice(t, []float64{0.7586521, -0.569575, -0.390976}, output[2].Value().Data(), 1.0e-05)
	...
	assert.InDeltaSlice(t, []float64{0.01654154, 0.48942297, -0.1587743, -0.2387454}, x1.Grad().Data(), 1.0e-05)
	assert.InDeltaSlice(t, []float64{0.04408108, 0.18716132, -0.15425818, -0.040870}, x2.Grad().Data(), 1.0e-05)
	assert.InDeltaSlice(t, []float64{0.05800791, 0.205784865, -0.2431444, -0.1281430}, x3.Grad().Data(), 1.0e-05)
