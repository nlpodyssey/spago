...
func TestModel_Forward(t *testing.T) {
	...
	assert.InDeltaSlice(t, []float64{-0.456097, -0.855358, -0.79552, 0.844718}, y.Value().Data(), 1.0e-05)

	// == Backward

	g.Backward(y, ag.OutputGrad(mat.NewVecDense([]float64{0.57, 0.75, -0.15, 1.64})))

	assert.InDeltaSlice(t, []float64{0.822396, 0.132595, -0.437002, 0.446894}, x.Grad().Data(), 1.0e-06)
