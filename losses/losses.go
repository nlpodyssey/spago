...
func MSE(x ag.Node, y ag.Node, reduceMean bool) ag.Node {
	loss := ag.ProdScalar(ag.Square(ag.Sub(x, y)), ag.Var(x.Value().NewScalar(float.Float(0.5))))
	if reduceMean {
		return ag.ReduceMean(loss)
	}
	return ag.ReduceSum(loss)
...
func FocalLoss(x ag.Node, c int, gamma float64) ag.Node {
	ce := CrossEntropy(x, c)
	p := ag.Exp(ag.Neg(ce))
	sub := ag.ReverseSub(p, ag.Var(x.Value().NewScalar(float.Float(1.0))))
	a := ag.Pow(sub, gamma)
	return ag.Prod(a, ce)
}
...
func ZeroOneQuantization(x ag.Node) ag.Node {
	return ag.ReduceSum(ag.Prod(ag.Square(x), ag.Square(ag.ReverseSub(x, ag.Var(x.Value().NewScalar(float.Float(1.0)))))))
}

// Norm2Quantization is a loss function that is minimized when norm2(x) = 1.
func Norm2Quantization(x ag.Node) ag.Node {
	return ag.Square(ag.SubScalar(ag.ReduceSum(ag.Square(x)), ag.Var(x.Value().NewScalar(float.Float(1.0)))))
}
...
func OneHotQuantization[T float.DType](x ag.Node, q T) ag.Node {
	return ag.ProdScalar(ag.Add(ZeroOneQuantization(x), Norm2Quantization(x)), ag.Var(x.Value().NewScalar(float.Float(q))))
}
...
func Distance[T float.DType](x ag.Node, target T) ag.Node {
	return ag.Abs(ag.Sub(ag.Var(x.Value().NewScalar(float.Float(target))), x))
}
...
func MSESeq(predicted []ag.Node, target []ag.Node, reduceMean bool) ag.Node {
	...
	if reduceMean {
		return ag.DivScalar(loss, ag.Var(loss.Value().NewScalar(float.Float(float64(len(predicted))))))
	}
	return loss
}
