...
func (m *Model) Forward(xs ...ag.Node) []ag.Node {
	...
	one := ag.Var(xs[0].Value().NewScalar(float.Interface(1.0)))
	k := ag.Var(xs[0].Value().NewScalar(float.Interface(0.1)))
	c := ag.Var(xs[0].Value().NewScalar(float.Interface(m.Scale)))
