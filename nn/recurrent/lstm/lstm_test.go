...
func testModelForwardWithPrev[T mat.DType](t *testing.T) {
	...
	// == Forward
	s0 := &State{
		Cell: ag.NewVariable(mat.NewVecDense([]T{0.8, -0.6, 1.0, 0.1, 0.1}), true),
		Y:    ag.NewVariable(mat.NewVecDense([]T{-0.2, 0.2, -0.3, -0.9, -0.8}), true),
	}
	x := ag.NewVariable(mat.NewVecDense([]T{-0.8, -0.9, -0.9, 1.0}), true)
	s1 := model.Next(s0, x)
...
func testModelForwardSeq[T mat.DType](t *testing.T) {
	...
	// == Forward
	s0 := &State{
		Cell: ag.NewVariable(mat.NewVecDense([]T{0.0, 0.0}), true),
