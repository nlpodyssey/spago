...
func testModelForward[T mat.DType](t *testing.T) {
	...
	y := s.Module().Forward(x)[0]

	assert.InDeltaSlice(t, []T{-0.39693, -0.796878, 0.0, 0.701374, -0.187746}, y.Value().Data(), 1.0e-05)

	// == Backward

	s.Graph().Backward(y, ag.OutputGrad[T](mat.NewVecDense([]T{0.57, 0.75, -0.15, 1.64, 0.45})))
...
func testModelForwardWithPrev[T mat.DType](t *testing.T) {
	...
	// == Forward
	x := s.NewVariable(mat.NewVecDense([]T{-0.8, -0.9, -0.9, 1.0}), true)
	yPrev := ag.Tanh(s.NewVariable(mat.NewVecDense([]T{-0.2, 0.2, -0.3, -0.9, -0.8}), true))
	s1 := s.Module().Next(&State[T]{Y: yPrev}, x)

	assert.InDeltaSlice(t, []T{-0.39693, -0.842046, 0.256335, 0.701374, 0.205456}, s1.Y.Value().Data(), 1.0e-05)

	// == Backward

	s.Graph().Backward(s1.Y, ag.OutputGrad[T](mat.NewVecDense([]T{0.57, 0.75, -0.15, 1.64, 0.45})))
...
func testModelForwardSeq[T mat.DType](t *testing.T) {
	...
	s0 := &State[T]{Y: s.NewVariable(mat.NewVecDense([]T{0.0, 0.0}), true)}
