...
func testModelForward[T float.DType](t *testing.T) {
	...
	gold := mat.NewDense[T](mat.WithBacking([]T{0.57, 0.75, -0.15, 1.64}))
...
func testModelForwardWithPrev[T float.DType](t *testing.T) {
	...
	yPrev := mat.NewDense[T](mat.WithBacking([]T{0.7, 0.6, 0.2, 0.8}), mat.WithGrad(true))
	cellPrev := mat.NewDense[T](mat.WithBacking([]T{0.574443, 0.425557, 0.401312, 0.524979}), mat.WithGrad(true))
...
func testModelForwardSeq[T float.DType](t *testing.T) {
	...
	// == Forward
	s0 := &State{
		Cell: mat.NewDense[T](mat.WithBacking([]T{0.0, 0.0, 0.0}), mat.WithGrad(true)),
		Y:    mat.NewDense[T](mat.WithBacking([]T{0.0, 0.0, 0.0}), mat.WithGrad(true)),
	}
	...
	s1.Y.AccGrad(mat.NewDense[T](mat.WithBacking([]T{-0.2, -0.3, -0.4})))
	s2.Y.AccGrad(mat.NewDense[T](mat.WithBacking([]T{0.6, -0.3, -0.2})))
